...
static esp_err_t handle_flash_begin(struct flash_begin_req *flash_begin, void *) {
...
static esp_err_t handle_flash_data(struct flash_data_req *flash_data, void *) {
	if (!flashing_info.is_active) {
		return ESP_ERR_INVALID_STATE;
	}
...
static esp_err_t handle_flash_data_end(void *, void *) {
	esp_err_t error;
...
static esp_err_t handle_configure_shutdown(struct cfg_shtdn_req *req, void *) {
...
static esp_err_t handle_configure_engagement(struct cfg_engage_req *req, void *) {
	struct node_spi_config config;
	esp_err_t error = load_node_hw(req->node_id, &config);
	if (error != ESP_OK) {
		return error;
...
static esp_err_t handle_scan(struct scan_req *req, recv_packet_t *inbound) {
	union msg response;
	...
	for (size_t i = 0; i < (sizeof(config.nodes) / sizeof(*config.nodes)); i++) {
		...
		send_msg(inbound->sender_mac_addr, &response);
	}
...
void app_main(void) {
	...
	for (;;) {
		...
		switch (req->type) {
			...
			case SCAN: result = handle_scan(&req->scan, &inbound); break;
			case SET_NODE_INFO: result = write_device_config(&req->set_node_info); break;
		}

		response->type = req->type;
		response->ok = result == ESP_OK;
