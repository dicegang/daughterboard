...
public abstract class SSEController implements Controller {
	...
	protected final void sendEvent(View event) throws IOException, InterruptedException {
		try {
			var body = exchange.get().getResponseBody();
			body.write("event: update\n".getBytes(StandardCharsets.UTF_8));
			body.write("data: ".getBytes(StandardCharsets.UTF_8));
			body.write(event.generateHTML().replace('\n', ' ').getBytes(StandardCharsets.UTF_8));
			body.write("\n\n".getBytes(StandardCharsets.UTF_8));
			body.flush();
	...
	public final void handleRequest(HttpsExchange exchange) {
		...
		try (exchange) {
			...
			logger.log(WARNING, "SSE connection died", e);
			return;
		} finally {
			if (activeCount != null)
