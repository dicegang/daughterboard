...
public class ProgramExploreController implements Controller {
	...
	public void handleRequest(HttpsExchange exchange) {
		try {
			...
			var index = context().queryParam("index").flatMap(n -> tryParseInteger(n))
				.orElseThrow(() -> HTTPException.of( BAD_REQUEST));

			var program = repository.getFlashEvent(teamID, index)
			...
			exchange.getResponseHeaders().add("Last-Modified", HTTP_DATE.format(program.start()));
