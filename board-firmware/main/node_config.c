...
static esp_err_t _write_spi_config(struct device_configuration *config) {
	...
	esp_err_t error = nvs_open(NVS_NAMESPACE_NAME, NVS_READWRITE, &handle);
	if (error != ESP_OK) {
		perror("nvs_open");
		return error;
	}

	error = nvs_set_blob(handle, NVS_KEY_NAME, config, sizeof(*config));
	if (error != ESP_OK) {
		perror("nvs_set_blob");
		goto done;
	}
...
static esp_err_t _load_spi_config(struct device_configuration *config) {
	...
	error = nvs_get_blob(handle, NVS_KEY_NAME, config, &length);
	if (error != ESP_OK) {
		ESP_LOGE(TAG, "error loading spi config: %s", esp_err_to_name(error));
...
static void init_spi_config(void) {
	...
	switch (err) {
		...
		case ESP_ERR_NVS_NOT_FOUND:
			...
			break;
		default:
			ESP_LOGE(TAG, "error loading spi config: %s", esp_err_to_name(err));
			abort();
	}
}
